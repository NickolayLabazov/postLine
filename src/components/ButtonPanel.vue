  ...
  <div class="buttonpanel-maincontainer">
    ...
    <div v-if="user!==null" class="buttonpanel-buttonbox">
      <b-button
        ...
        v-on:click="handleClaps()"
      >{{post.claps}} хлопать</b-button>
      <b-button
        type="is-primary"
        size="is-small"
        outlined
    ...
    dateNow() {
      ...
      try {
        if (this.post.updateAt !== null) {
          postDate = `${this.post.updateAt.slice(
            0,
            4
          )}${this.post.updateAt.slice(5, 7)}${this.post.updateAt.slice(
            8,
            10
          )}${this.post.updateAt.slice(11, 13)}${this.post.updateAt.slice(
            14,
            16
          )}`;
        } else {
          postDate = `${this.post.createdAt.slice(
            0,
            4
          )}${this.post.createdAt.slice(5, 7)}${this.post.createdAt.slice(
            8,
            10
          )}${this.post.createdAt.slice(11, 13)}${this.post.createdAt.slice(
            14,
            16
          )}`;
        }
